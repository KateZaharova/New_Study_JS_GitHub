...
console.log(elements.form);
 ...
 async function getDataTimezones() {
...
let timezones = getDataTimezones();//.map((v) => {
   // `<option value=${v.id}> ${v.utc_offset} ${v.timezone} ${v.name} </option>`;
//}).join();
        
console.log(timezones);


                //$('#id').append('<option value="v.id">' + v.utc_offset + v.timezone + v.name + '</option>');
           // });   
